...
def runOptimizedOCSVM_CV(data_folder, cfg):
    ...
    fig.savefig('OCSVM/' + "DeltaShaper_OCSVM_" + cfg[1] + ".pdf")   # save the figure to file
    plt.close(fig)



def ocsvmCV(nu, gamma):
...
def runOCSVM_BO(data_folder,cfg):
    gp_params = {"alpha": 1e-5}

    ocsvmBO = BayesianOptimization(ocsvmCV, {'nu':(0.01,1), 'gamma':(0.01,100)})

    ocsvmBO.maximize(init_points=5, n_iter=10, acq="ei", xi=0.1, **gp_params)

    print('Final Results')
    print('OCSVM max: %f' % ocsvmBO.res['max']['max_val'])
    print('OCSVM avg: %f' % np.mean(ocsvmBO.res['all']['values'],axis=0))
