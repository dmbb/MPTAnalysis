...
def runANN_FixedParams(data_folder,cfg):
    epochs = 1000
    #Gather the dataset
...
def runANNSearch(data_folder,cfg):
    ...
    n_hiddens = [4, 8, 16, 32, 64, 128, 256, 512]#np.logspace(2, 10, base=2, num=12)
    #drop_inputs = [0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1]
    #drop_hiddens = [0.5, 0.6, 0.7, 0.8, 0.9, 1]
    repr_sizes = [4, 8, 16, 32, 64, 128, 256, 512] #np.logspace(2, 10, base=2, num=12) #num 20
    ...
    fig.savefig('Autoencoder/' + "DeltaShaper_Autoencoder_" + cfg[1] + ".pdf")   # save the figure to file
    plt.close(fig)
...
def runANN_BO(data_folder,cfg):
    ...
    annBO = BayesianOptimization(annCV, {'n_hidden':(4,256), 'repr_size':(4,256)})

    #1e-4 - exploitation
    #0.1 - exploration
    #1e-2 - balance between exploration and exploitation
