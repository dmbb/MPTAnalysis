    ...
    for sample in os.listdir(sampleFolder):
        ...
        for ts, buf in pcap:
            ...
            try:
                ...
                if (ip_hdr.p == 17 and (ip_hdr.data.sport == 443 or ip_hdr.data.dport == 443)):
                    ...
                    if(ip_hdr.data.sport == 443):
                        ...
                        if(current_burst != 0):
                            if(current_burst > 1):
                                bursts_packets.append(current_burst) #packets on burst
                                burst_sizes.append(current_burst_size) #total bytes on burst
        ...
        bin_list = []
        for i in od_dict:
            bin_list.append(od_dict[i]/float(len(packetSizesOut))) #Fraction of packets inside a given bin
        """
        ...
        totalBursts = len(bursts_packets)
        meanBurst = np.mean(bursts_packets)
        medianBurst = np.median(bursts_packets)
        stdevBurst = np.std(bursts_packets)
        varianceBurst = np.var(bursts_packets)
        maxBurst = np.amax(bursts_packets)
        kurtosisBurst = kurtosis(bursts_packets)
        skewBurst = skew(bursts_packets)
        p10Burst = np.percentile(bursts_packets,10)
        p20Burst = np.percentile(bursts_packets,20)
        p30Burst = np.percentile(bursts_packets,30)
        p40Burst = np.percentile(bursts_packets,40)
        p50Burst = np.percentile(bursts_packets,50)
        p60Burst = np.percentile(bursts_packets,60)
        p70Burst = np.percentile(bursts_packets,70)
        p80Burst = np.percentile(bursts_packets,80)
        p90Burst = np.percentile(bursts_packets,90)


        ########################################################################
        # Statistical indicators for Outgoing bytes (sliced intervals)
        meanBurstBytes = np.mean(burst_sizes)
        medianBurstBytes = np.median(burst_sizes)
        stdevBurstBytes = np.std(burst_sizes)
        varianceBurstBytes = np.var(burst_sizes)
        kurtosisBurstBytes = kurtosis(burst_sizes)
        skewBurstBytes = skew(burst_sizes)
        maxBurstBytes = np.amax(burst_sizes)
        minBurstBytes = np.amin(burst_sizes)
        p10BurstBytes = np.percentile(burst_sizes,10)
        p20BurstBytes = np.percentile(burst_sizes,20)
        p30BurstBytes = np.percentile(burst_sizes,30)
        p40BurstBytes = np.percentile(burst_sizes,40)
        p50BurstBytes = np.percentile(burst_sizes,50)
        p60BurstBytes = np.percentile(burst_sizes,60)
        p70BurstBytes = np.percentile(burst_sizes,70)
        p80BurstBytes = np.percentile(burst_sizes,80)
        p90BurstBytes = np.percentile(burst_sizes,90)
        ...
        f_names.append('totalBursts')
        f_values.append(totalBursts)
        f_names.append('maxBurst')
        f_values.append(maxBurst)
        f_names.append('meanBurst')
        f_values.append(meanBurst)
        #f_names.append('medianBurst')
        #f_values.append(medianBurst)
        f_names.append('stdevBurst')
        f_values.append(stdevBurst)
        f_names.append('varianceBurst')
        f_values.append(varianceBurst)
        f_names.append('kurtosisBurst')
        f_values.append(kurtosisBurst)
        f_names.append('skewBurst')
        f_values.append(skewBurst)

        f_names.append('p10Burst')
        f_values.append(p10Burst)
        f_names.append('p20Burst')
        f_values.append(p20Burst)
        f_names.append('p30Burst')
        f_values.append(p30Burst)
        f_names.append('p40Burst')
        f_values.append(p40Burst)
        f_names.append('p50Burst')
        f_values.append(p50Burst)
        f_names.append('p60Burst')
        f_values.append(p60Burst)
        f_names.append('p70Burst')
        f_values.append(p70Burst)
        f_names.append('p80Burst')
        f_values.append(p80Burst)
        f_names.append('p90Burst')
        f_values.append(p90Burst)


        #################################################################
        #Packet Bursts data size features
        f_names.append('maxBurstBytes')
        f_values.append(maxBurstBytes)
        f_names.append('minBurstBytes')
        f_values.append(minBurstBytes)
        f_names.append('meanBurstBytes')
        f_values.append(meanBurstBytes)
        f_names.append('medianBurstBytes')
        f_values.append(medianBurstBytes)
        f_names.append('stdevBurstBytes')
        f_values.append(stdevBurstBytes)
        f_names.append('varianceBurstBytes')
        f_values.append(varianceBurstBytes)
        f_names.append('kurtosisBurstBytes')
        f_values.append(kurtosisBurstBytes)
        f_names.append('skewBurstBytes')
        f_values.append(skewBurstBytes)

        f_names.append('p10BurstBytes')
        f_values.append(p10BurstBytes)
        f_names.append('p20BurstBytes')
        f_values.append(p20BurstBytes)
        f_names.append('p30BurstBytes')
        f_values.append(p30BurstBytes)
        f_names.append('p40BurstBytes')
        f_values.append(p40BurstBytes)
        f_names.append('p50BurstBytes')
        f_values.append(p50BurstBytes)
        f_names.append('p60BurstBytes')
        f_values.append(p60BurstBytes)
        f_names.append('p70BurstBytes')
        f_values.append(p70BurstBytes)
        f_names.append('p80BurstBytes')
        f_values.append(p80BurstBytes)
        f_names.append('p90BurstBytes')
        f_values.append(p90BurstBytes)
...
if __name__ == "__main__":
    sampleFolders = [
    "TrafficCaptures/YouTube_home_world_live",
    "TrafficCaptures/CovertCast_home_world"
    ]
