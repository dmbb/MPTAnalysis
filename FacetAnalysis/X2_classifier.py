...
def buildModels(freq_dists):
    ...
    for i in range(0, len(model_censored)):
        ...
        for v in freq_dists[1]:
            ...
            v_total_grams = sum(v)
            total_ngrams_censored_set += sum(v)
            somatory += (v_total_grams * n_gram_prob)
...
def Prepare_BadX_Fixed(fig_folder, cfg,binWidth,freq_dists):
    ...
    chat_samples = freq_dists[0]
    censored_samples = freq_dists[1]
    bad_censored_samples = freq_dists[2]

    filtered_freq_dists = []
    filtered_chat_samples = []
    filtered_censored_samples = []
    filtered_bad_censored_samples = []

    if(optimization):
        #Optimize bigram choice, build updated frequency distributions
        indexes_to_remove = optimizeBigrams(freq_dists[:2])
        ...
        for sample in bad_censored_samples:
            filtered_bad_censored_samples.append(np.delete(sample, indexes_to_remove))
    else:
        #Ignore optimization procedure, carry on with original frequency distributions
        filtered_chat_samples = chat_samples
        filtered_censored_samples = censored_samples
    ...
    filtered_freq_dists1 = []
    filtered_freq_dists2 = []
    filtered_freq_dists3 = []
    filtered_freq_dists4 = []
    ...
    filtered_freq_dists3.append(filtered_chat_samples[:len(filtered_chat_samples)/2:])
    filtered_freq_dists3.append(filtered_bad_censored_samples[:len(filtered_bad_censored_samples)/2:])
...
def X_BadClassify_Fixed(cfg, binWidth, freq_dists, model_chat, model_censored):
    ...
    FNR = FNegatives/(TPositives+float(FNegatives)+eps)
    TPR = TPositives/(TPositives+float(FNegatives)+eps)
    FPR = FPositives/(FPositives+float(TNegatives)+eps)
    PPV = TPositives/(TPositives+float(FPositives)+eps)
...
def Prepare_X_RatioReproduction(fig_folder, cfg,binWidth,freq_dists):
    ...
    if(optimization):
        ...
        #Optimize bigram choice, build updated frequency distributions
        indexes_to_remove = optimizeBigrams(freq_dists)
        np.save(fig_folder + "RemovedIndexes_" + cfg[1], np.array(indexes_to_remove))
    ...
    np.save(fig_folder + "ROC_" + str(optimization) + "_" + cfg[1] + "_" + str(binWidth)+"_Sensitivity", np.array(Sensitivity))
    np.save(fig_folder + "ROC_" + str(optimization) + "_" + cfg[1] + "_" + str(binWidth)+"_Specificity", np.array(Specificity))
    """
    ...
    ax1.plot(1 - Specificity, Sensitivity, 'k.-', color='black', label = 'Mean ROC (AUC = %0.2f)' % (auc))
    ax1.plot([0, 1], [0, 1], 'k--', lw=2, color='orange', label = 'Random Guess')
    ax1.grid(color='black', linestyle='dotted')

    #plt.title('Receiver Operating Characteristic (ROC)')
    plt.xlabel('False Positive Rate', fontsize='xx-large') #one size down
    plt.ylabel('True Positive Rate', fontsize='xx-large')
    plt.legend(loc='lower right', fontsize='x-large')

    plt.setp(ax1.get_xticklabels(), fontsize=16) #14
...
if __name__ == "__main__":
    ...
    sampleFolder = "TrafficCaptures/240Resolution/"
    ...
    """
    #For simulating a censor using a classifier trained for a given anomaly class when classifying another.
    for cfg in bad_cfgs:
        random.seed(a=1) # re-seed
        print "====================================="
        print "X classifier - Regular Trained with " + cfg[1] + " vs " + cfg[2]
        for binWidth in BIN_WIDTH:
            print "---------------------"
            print "Bin Width: " + str(binWidth)
            print "---------------------"
            ...
            x = random.sample(freq_dists[0], len(freq_dists[0]))
            x2 = random.sample(freq_dists[1], len(freq_dists[1]))
            x3 = random.sample(freq_dists[2], len(freq_dists[2]))
            freqs = []
            freqs.append(x)
            freqs.append(x2)
            freqs.append(x3)
            ...
            #For getting fixed classification rates to compare with classifiers without a notion of internal thereshold
            Prepare_BadX_Fixed(fig_folder, cfg,binWidth, freqs)
    """
